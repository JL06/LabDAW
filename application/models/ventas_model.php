...
class Ventas_model extends Generic_model{
	function get_ventas($filter=""){
			$this->db->select('ventas.id,productos.nombre as producto,tipoproducto.nombre as tipo,usuario.nombre as usuario ,lugar.nombre as lugar, precio,fecha, cantidad, cantidad * precio as total');
			$this->db->from('ventas');
